...
onload = function(){
	...
	var u_RotateM = gl.getUniformLocation(prg, "u_RotateM");
	...
	var q = new qtnIV()
	var rotateQ = q.identity(q.create());
	var earthAxis = [0.0, 1.0, 0.0];	// 地軸
	...
	(function(){
		...
		else if (120 <= count && count < 180) {
			axis = earthAxis;
			rad = Math.PI / 180;
		}
		else if (180 <= count && count < 300) {
			axis = [1.0, 0.0, 0.0];
			rad = -Math.PI / 180;
		}
		else if (300 <= count && count < 360) {
			axis = earthAxis;
			rad = -Math.PI / 180;
		}
		else if (360 <= count) {
		...
		q.rotate(rad, axis, tempQ);					// 回転を生成
		q.multiply(rotateQ, tempQ, rotateQ);		// 回転を適用
		q.toVecIII(earthAxis, tempQ, earthAxis);	// 地軸に回転を適用
		q.toMatIV(rotateQ, modelM);					// 回転行列に変換
		...
		++count;
		setTimeout(arguments.callee, 1000 / 60);	// 60フレームでループ
	})();
...
function create_sphere(row, column, rad) {
	...
	for (var i = 0; i <= row; ++i) {
		...
		for (var j = 0; j <= column; ++j) {
			var phi = (2.0 * Math.PI) * (j / column) - Math.PI;
